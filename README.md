...
netflix_df = pd.DataFrame(movie_dict)
durations_df = pd.DataFrame(movie_dict, index=years)
...
<p>Upon asking our friend for the original CSV they used to perform their analyses, they gladly oblige and sent it. We now have access to the CSV file, available at the path <code>"datasets/netflix_data.csv"</code>. Let's create another DataFrame, this time with all of the data. Given the length of our friend's data, printing the whole DataFrame is probably not a good idea, so we will inspect it by printing only the first five rows.</p>
...
are_movies_getting_shorter = "No, we can't know for certain until we perform additional analyses."
